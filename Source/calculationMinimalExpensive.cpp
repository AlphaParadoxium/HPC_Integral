...
{
    return (sin(pow((begin + (stepSize*workPackageNr)),2)) + pow((begin + (stepSize*workPackageNr)),3) + 10*(begin + (stepSize*workPackageNr)) + cos(30) + sin(10*(begin + (stepSize*workPackageNr)))) * stepSize;
}
...
int main(){
    ...
    #pragma omp parallel for reduction(+:calculationResult)
